...
export function ConnectWebsite() {
  ...
  // Fetch user's API key prefix for the snippet
...
(function() {
  ...
  // Real-time verification: listen for new tracking events matching the user's site
  const verifyConnection = useCallback(async () => {
    if (!verifyUrl.trim() || !profile?.id) return;
    setVerifying(true);
    setVerifyResult(null);

    // Check if any recent events contain this URL in the payload
    ...
    if (events && events.length > 0) {
      ...
      // Also check via cast since payload is JSONB
  ...
  // Subscribe to realtime events for instant verification
  useEffect(() => {
        ...
        <TabsContent value="script">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Add this snippet to every page of your website, just before the closing &lt;/head&gt; tag. Works with any website.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="relative">
                ...
                <Button
                  ...
                  onClick={() => copyToClipboard(universalSnippet, 'Universal script')}
                >
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-3 rounded-lg border bg-muted/30">
                <p className="text-xs text-muted-foreground">
                  <strong>Pro tip:</strong> For server-side GTM, set your sGTM container URL as a custom domain in NexusTrack Settings. This routes all events through your first-party domain for maximum data quality.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="api">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Send events directly from your backend using our REST API or SDKs.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="relative">
    ...
    "data": {
      ...
      "items": [{"sku": "PROD-1", "quantity": 1, "price": 99.99}]
                ...
                <Button
                  ...
                  variant="outline"
                  className="absolute top-2 right-2"
                  onClick={() => copyToClipboard(`curl -X POST ${API_BASE_URL}/webhook-trigger -H "Content-Type: application/json" -H "x-api-key: ${apiKey || 'YOUR_API_KEY'}" -d '{"event":"purchase","data":{"order_id":"ORD-12345","email":"customer@example.com","value":99.99}}'`, 'cURL command')}
                >
